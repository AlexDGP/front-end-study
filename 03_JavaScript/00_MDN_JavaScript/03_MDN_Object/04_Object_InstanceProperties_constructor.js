...
/* Description */
// Any object (with the exception of objects created with Object.create(null)) will have a constructor property on its [[Prototype]].
// Objects created without the explicit use of a constructor function (such as object literals and array literals) will have a constructor property that points to the Fundamental Object constructor type for that object.
...
console.log(arr.hasOwnProperty("constructor")); // true
...
const arr = [];
console.log(Object.hasOwn(arr, "constructor")); // false
console.log(Object.hasOwn(Object.getPrototypeOf(arr), "constructor")); // true

arr.constructor = String;
Object.hasOwn(arr, "constructor"); // true — the instance property shadows the one on its prototype

// But even when Object.getPrototypeOf(a).constructor is re-assigned, it won't change other behaviors of the object.
// For example, the behavior of instanceof is controlled by Symbol.hasInstance, not constructor:
const arr = [];
arr.constructor = String;
arr instanceof String; // false
arr instanceof Array; // true

// There is nothing protecting the constructor property from being re-assigned or shadowed,
// so using it to detect the type of a variable should usually be avoided in favor of less fragile ways like instanceof and Symbol.toStringTag for objects, or typeof for primitives.

// Changing the constructor of a constructor function's prototype
// Every function constructor will have a prototype property, which will become the instance's [[Prototype]] when called via the new operator.
// ConstructorFunction.prototype.constructor will therefore become a property on the instance's [[Prototype]], as previously demonstrated.
...
// Ensuring that Child.prototype.constructor always points to Child itself is crucial when you are using constructor to access the original class from an instance.
